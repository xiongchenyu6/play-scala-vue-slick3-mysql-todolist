...
class PasswordInfoDAO @Inject() (protected val dbConfigProvider: DatabaseConfigProvider)
  ...
  protected def passwordInfoQuery(loginInfo: LoginInfo) = for {
    ...
    dbPasswordInfo <- Passwordinfo if dbPasswordInfo.logininfoid === dbLoginInfo.id
  ...
  protected def passwordInfoSubQuery(loginInfo: LoginInfo) =
    Passwordinfo.filter(_.logininfoid in loginInfoQuery(loginInfo).map(_.id))
  ...
  protected def addAction(loginInfo: LoginInfo, authInfo: PasswordInfo) =
    loginInfoQuery(loginInfo).result.head.flatMap { dbLoginInfo =>
      Passwordinfo +=
        PasswordinfoRow(authInfo.hasher, authInfo.password, authInfo.salt, dbLoginInfo.id)
    }.transactionally
  ...
  def save(loginInfo: LoginInfo, authInfo: PasswordInfo): Future[PasswordInfo] = {
    val query = loginInfoQuery(loginInfo).joinLeft(Passwordinfo).on(_.id === _.logininfoid)

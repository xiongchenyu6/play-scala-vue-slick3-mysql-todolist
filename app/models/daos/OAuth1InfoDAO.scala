...
class OAuth1InfoDAO @Inject() (protected val dbConfigProvider: DatabaseConfigProvider)
    extends DelegableAuthInfoDAO[OAuth1Info] with HasDatabaseConfigProvider[JdbcProfile] {
  ...
  protected def oAuth1InfoQuery(loginInfo: LoginInfo) = for {
    ...
    dbOAuth1Info <- Oauth1info if dbOAuth1Info.logininfoid === dbLoginInfo.id
  } yield dbOAuth1Info
  ...
  protected def oAuth1InfoSubQuery(loginInfo: LoginInfo) =
    Oauth1info.filter(_.logininfoid in loginInfoQuery(loginInfo).map(_.id))

  protected def addAction(loginInfo: LoginInfo, authInfo: OAuth1Info) =
    loginInfoQuery(loginInfo).result.head.flatMap { dbLoginInfo =>
      Oauth1info += Oauth1infoRow(0, authInfo.token, authInfo.secret, dbLoginInfo.id)
    }.transactionally
  ...
  def save(loginInfo: LoginInfo, authInfo: OAuth1Info): Future[OAuth1Info] = {
    val query = loginInfoQuery(loginInfo).joinLeft(Oauth1info).on(_.id === _.logininfoid)
